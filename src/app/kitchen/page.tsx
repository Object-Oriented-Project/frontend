...
export default function KitchenPage() {
    ...
    return (
        <div> 
            <div> Your Job</div>
            <div>
                <button className="btn btn-secondary" onClick={() => handleFilterClick('All')}>All</button>
                <button className="btn btn-secondary" onClick={() => handleFilterClick('Done')}>Done</button>
                <button className="btn btn-primary"onClick={() => handleFilterClick('Ongoing')}>Ongoing</button>
            </div>
            <div>
            {filterOrders(orders, filterStatus).map(order => (
                    <div key={order.id} className="container sm mb-4 bg-red-200 flex flex-col ml-5 w-80">
                        <div className="flex flex-row justify-between items-center rounded-md bg-blue-300">
                            <span className="font-semibold">{order.name}</span>
                            <p>
                                <select className="select select-bordered w-full max-w-xs" value={order.status} onChange={e => handleStatusChange(order.id, e.target.value)}>
                                    <option value="Done">Done</option>
                                    <option value="Ongoing">Ongoing</option>
                                </select>
                            </p>
                            <button className=" bg-gray-200 text-left p-2 mb-2" onClick={() => toggleOrderDetails(order.id)}>
